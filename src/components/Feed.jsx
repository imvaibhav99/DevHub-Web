...
import axios from 'axios';
import { addFeed } from '../utils/feedSlice';
import UserCard from './userCard.jsx'; // Ensure this is the correct path
...
const Feed = () => {
  ...
  const [page, setPage] = useState(1); // For pagination/infinite scroll
  ...
  const fetchFeed = useCallback(async (pageNum) => {
    ...
    try {
      ...
      dispatch(addFeed(res.data));
      setHasMore(res.data.length > 0); // Assume API returns fewer items when no more data
    } catch (err) {
      console.error('Error fetching feed:', err);
  ...
  return (
    <div className="min-h-screen w-full flex flex-col items-center px-2 bg-gradient-to-br from-gray-950 via-purple-950/80 to-[#120922]">
      <h1 className="text-4xl font-bold my-8 text-center bg-gradient-to-r from-purple-300 to-pink-300 bg-clip-text text-transparent drop-shadow">
        My Feed
      </h1>
      ...
      {error ? (
        ...
        <div className="animate-fade-in-up grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 w-full max-w-6xl px-4">
        ...
        <div className="flex flex-col items-center justify-center p-8 rounded-2xl bg-white/10 backdrop-blur-md shadow-lg mt-12">
          <svg className="w-16 h-16 text-purple-300 mb-4" viewBox="0 0 24 24" fill="none" aria-hidden="true">
            <path
              fill="currentColor"
              d="M17.5 2a2.5 2.5 0 110 5 2.5 2.5 0 010-5zM6.5 2a2.5 2.5 0 110 5 2.5 2.5 0 010-5zM21 20.875C21 18.657 16.107 17 12 17s-9 1.657-9 3.875C3 22.142 5.927 23 12 23c6.073 0 9-.858 9-2.125zm1-14.25c0-2.338-7-3.625-10-3.625S2 4.287 2 6.625c0 1.012 1.424 1.985 4.133 2.56a15.033 15.033 0 00.647 2.095C3.606 11.763 2 13.198 2 15c0 2.138 5.286 3.875 10 3.875s10-1.737 10-3.875c0-1.802-1.606-3.237-4.78-4.72.19-.633.444-1.388.647-2.095C21.576 8.61 23 7.637 23 6.625z"
            />
          </svg>
          <h2 className="text-2xl font-semibold text-gray-200 mb-2">No New Users Found</h2>
          <p className="text-gray-400 text-center max-w-xs mb-4">
            Looks like you've reached the end of your feed! Check back later for new developer matches.
          </p>
        </div>
      )}

      {loading && (
        <div className="flex flex-col items-center gap-2 my-8" aria-busy="true" role="status">
          <span className="loading loading-spinner loading-lg text-purple-500"></span>
          <span className="text-purple-200 text-lg animate-pulse">Fetching more dev matches...</span>
        </div>
      )}
      ...
      <style>{`
        ...
        .animate-fade-in-up {
          animation: fade-in-up 0.66s cubic-bezier(0.32, 1.1, 0.36, 1) both;
        }
      `}</style>
    </div>
  );
