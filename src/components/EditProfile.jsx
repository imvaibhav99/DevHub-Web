...
const EditProfile = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    // Build social links only with non-empty values
    const socialLinksPayload = {};
    if (github?.trim()) socialLinksPayload.github = github.trim();
    if (linkedIn?.trim()) socialLinksPayload.linkedIn = linkedIn.trim();
    if (x?.trim()) socialLinksPayload.x = x.trim();
    if (leetcode?.trim()) socialLinksPayload.leetcode = leetcode.trim();
    if (gfg?.trim()) socialLinksPayload.gfg = gfg.trim();

    // Build payload; omit optional empty fields
    const payload = {
      ...
      ...(gender ? { gender } : {}),
      age: ageNumber,
      ...(formattedSkills.length ? { skills: formattedSkills } : {}),
      ...(Object.keys(socialLinksPayload).length ? { socialLinks: socialLinksPayload } : {}),
    };
    ...
    try {
      ...
      if (err.response) {
        ...
        message = (typeof data === "string" && data) || data?.message || `Server error: ${err.response.status} - ${err.response.statusText}`;
      } else if (err.request) {
